...
set group = $8
set sigma = 5
set depthaper = ( 0.5 1.0 1.5 2.0 ) # arcsecond aperture for calculating depth. 
...
set BACKSIG = `gethead $coadd backsig backsigc`
...
if ( $SEXFLAG ) then
    set AVE1 = `awk 'BEGIN {ave=0;sig=0;i=0}{if( ($13*0.88*('$PIXS'/'$FWHM')**2 < '$SATURATE2') && $20 > '$stellarity' && $18==0  ){ave=ave+($6-$16+'$CORR'+'$OFFSET')/($7)^2;sig=sig+1/($7)^2;i++}}END{if(i>0 && sig>0){printf "%7.3f %7.3f %d\n",ave/sig,sqrt(1/sig),i}else{print 0,0,0}}' 2coadd_${filter}_${group}.coo`
echo $AVE1[1]
    set nmed = `awk 'BEGIN{ print int('$AVE1[3]'/2+1)}'`
...
#    set AVE1 = `awk 'BEGIN {ave=0;sig=0;i=0}{if( ($13*0.88*('$PIXS'/'$FWHM')**2 < '$SATURATE2') && $20 > '$stellarity' ){ave=ave+($6-$16)+'$CORR'+'$OFFSET';sig=sig+($6-$16)^2;i++}}END{if(i>0){printf "%7.3f %7.3f %d\n",ave/i,sqrt(sqrt((i*sig-ave^2)^2))/i,i}else{print 0,0,0}}' 2coadd_${filter}_${group}.coo`
    set AVE1 = `awk 'BEGIN {ave=0;sig=0;i=0}{if( ($13*0.88*('$PIXS'/'$FWHM')**2 < '$SATURATE2') && $20 > '$stellarity' ){ave=ave+($6-$16+'$CORR'+'$OFFSET')/($7)^2;sig=sig+1/($7)^2;i++}}END{if(i>0 && sig>0){printf "%7.3f %7.3f %d\n",ave/sig,sqrt(1/sig),i}else{print 0,0,0}}' 2coadd_${filter}_${group}.coo`
#
...
if ( `echo "$num < 3" | bc` ) then
    ...
    set MZP = `awk 'BEGIN {ave=0;sig=0;i=0}{if($2>0){ave=ave+($4/$2^2);sig=sig+1/($2)^2;i++}}END{if(i>0 && sig>0){printf "%7.3f %7.3f %d\n",ave/sig+'$CORR'+'$OFFSET',sqrt(1/sig),i}else{print 0,0,0}}' 2coadd_${filter}_${group}.mag`
endif

set BACKMAG = `echo "scale=3 ; -2.5*l($BACK / $PIXS / $PIXS)/l(10) + $MZP[1] " | bc -l`
#set DEPTH = `awk 'BEGIN {printf "%7.3f\n",-2.5*log(5*'$BACKSIG[2]'*3.1415926*((('$FWHM'/1)/2)/('$PIXS'))**2)/log(10) + '$MZP[1]' }'`
set DEPTH = `echo $depthaper | awk 'BEGIN {RS=" "}{printf "%7.3f\n",-2.5*log(5*'$BACKSIG[2]'*3.1415926*(($1/2)/('$PIXS'))**2)/log(10) + '$MZP[1]' }'`
printf " ------------------------------------\n"
printf "%20s: %7.3f+-%5.3f, n=%d\n" "2MASS after clipping" $MZP[1] $MZP[2] $MZP[3]
printf "%20s= %7.3f [mag per sq-arcsecond]\n" "SKY BACKGROUND" $BACKMAG
printf "%20s= %7.3f ["$depthaper[2]"-arcsecond aperture]\n " ""$sigma"-sigma DEPTH" $DEPTH[2]
printf " ------------------------------------\n"
...
sethead $coadd DEPTH05\=$DEPTH[1] / ""$sigma"-sigma depth ["$depthaper[1]"-arcsec aperture]"
sethead $coadd DEPTH10\=$DEPTH[2] / ""$sigma"-sigma depth ["$depthaper[2]"-arcsec aperture]"
sethead $coadd DEPTH15\=$DEPTH[3] / ""$sigma"-sigma depth ["$depthaper[3]"-arcsec aperture]"
sethead $coadd DEPTH20\=$DEPTH[4] / ""$sigma"-sigma depth ["$depthaper[4]"-arcsec aperture]"
...
#	set DEPTH = `awk 'BEGIN {printf "%7.3f\n",-2.5*log(5*'$BACKSIG[2]'*3.1415926*((('$FWHM'/1)/2)/('$PIXS'))**2)/log(10) + '$MZP[1]' }'`
	...
	sethead $coadd DEPTH05_$STN\=$DEPTH[1] / ""$sigma"-sigma depth ["$depthaper[1]"-arcsec aperture]"
	sethead $coadd DEPTH10_$STN\=$DEPTH[2] / ""$sigma"-sigma depth ["$depthaper[2]"-arcsec aperture]"
	sethead $coadd DEPTH15_$STN\=$DEPTH[3] / ""$sigma"-sigma depth ["$depthaper[3]"-arcsec aperture]"
	sethead $coadd DEPTH20_$STN\=$DEPTH[4] / ""$sigma"-sigma depth ["$depthaper[4]"-arcsec aperture]"
	set MZP[3] = 0
    endif
end
