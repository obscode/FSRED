...
set yaxis = 2048
set BADBITS = 127
# number of re-attempts if failure detected.
@ tryagain = 2
...
#		exit 0
	...
	if ( $tryflag == 1 ) then
	    if ( $bflag == 0 ) then
		if ( -e $name ) then
		    set TBACK = `gethead -u $name $BTYPE | awk '{if($1+0 == $1){print 1}else{print 0}}'`
		    if ( $TBACK != "0" ) then
		...
		if ( -e $name && -e $name.sky.fits ) then
		    set TBACK = `gethead -u $name $BTYPE | awk '{if($1+0 == $1){print 1}else{print 0}}'`
		    set TBACK2 = `gethead -u $name.sky.fits $BTYPE | awk '{if($1+0 == $1){print 1}else{print 0}}'`
		    if ( $TBACK != "0" && $TBACK2 != "0" ) then
			echo  "$name          $BTYPE = $TBACK "
			echo  "$name.sky.fits $BTYPE = $TBACK2 "
			continue
		    else
#			printf " Error : Are there enough background pixels in $name.  Exiting. \n"
#			exit 1
			printf " Warning : Not enough background pixels in $name. Using mimstat without masking... \n"
			sethead $name NOSKY=1
			rm -fr $name.sky.fits >>& /dev/null
		    endif
		else
		    printf "Error no $name.sky.fits file found... Exiting\n"
		    exit 1
		endif
	    endif
	endif
...
#	    goto imstat
		...
		if ( $weight == 1 ) then
		    ...
		    $tempdir/MJD_$ch.cat | sort -g -k2 | head -n $NBACK > $tempdir/z$name.dat
...
#			sedi 's|imcombine.lsigma = 3|imcombine.lsigma = 2|' $tempdir/z$name.para
		...
		# 32 - objects
		# 16 - transient
		# 8 - latent   <--- leave in, since it is present in the background.  NOPE, nevermind.
		# 4 - probes
		# 2 - saturated
		...
		# APPLIES ONLY TO REJECTION ALGORITHM, NOT MASKS
			...
			set cmd = "$cmd ; images imcombine \@$tempdir/z$name.para"
		    else
			if ( $ADVBACK == 1 ) then
			    set xmax = BAVE
			else
			...
			if ( `echo "$bflag < 1" | bc` ) then
			    set cmd = "$cmd ; $fsiraf/x_skycombine -i "$tempdir/z$name.datb" -m ".pl.fits" -o "$pwd/$name.sky.fits" -b "$pwd/$name.sky.exp.pl.fits" -p -1000 -q $xmax -l 5 -h 5 -v 1 -s 3 3 -1 -z "$tempdir/z$name.data" -y $BADBITS -n 0 -x $bflag -t 1 "
			else
			    set cmd = "$cmd ; $fsiraf/x_skycombine -i "$tempdir/z$name.datb" -m ".pl.fits" -o "$pwd/$name.sky.fits" -b "$pwd/$name.sky.exp.pl.fits" -p -1000 -q $xmax -l 5 -h 5 -v 1 -s 3 3 -1 -z "$tempdir/z$name.data" -y $BADBITS -n $bflag -x $bflag -t 1 "
			endif
		    endif
...
# TRY A SECOND TIME, CHECK THAT THE $BTYPE KEYWORD WAS CREATED
    ...
    if ( $tryflag == 0 ) then
	set tryflag = 1
	printf "\n verifying images are there setting tryflag = $tryflag  \n"
	sleep 1
	goto startloop
    else
	set tryflag = 0
...
#	    set scale = `paste $tempdir/z$name.zobj.txt $tempdir/z$name.zsky.txt | awk '{print $1/$2}' | sort -g | head -n 1`
#	    set diff = `paste $tempdir/z$name.zobj.txt $tempdir/z$name.zsky.txt | awk '{print $1-$2}' | sort -g | head -n 1`
	    set scale = `paste $tempdir/z$name.zobj.txt $tempdir/z$name.zsky.txt | awk '{a=(a*(NR-1)+$1/$2)/NR}END{print a}' | sort -g | head -n 1`
	    set diff = `paste $tempdir/z$name.zobj.txt $tempdir/z$name.zsky.txt | awk '{a=(a*(NR-1)+$1-$2)/NR}END{print a}' | sort -g | head -n 1`
...
if ( \`$gethead -u $name NOSKY\` != 1 ) then
...
# CANNOT USE MAP_WEIGHT WITH FLAG IMAGE IF THEY ARE THE SAME IMAGE.   ONLY ONE OR THE OTHER.
		    $sex $name.sub.fits -c $fsast/sex.config -PARAMETERS_NAME $fsast/sex.param -FILTER_NAME $fsast/default.conv -STARNNW_NAME $fsast/default.nnw -CATALOG_TYPE ASCII -CATALOG_NAME $name.sub.sex1 -VERBOSE_TYPE QUIET -WEIGHT_TYPE NONE -WEIGHT_IMAGE \$bpmfit.1.fits -CHECKIMAGE_TYPE NONE -CHECKIMAGE_NAME $name.check.fits -BACK_SIZE 32 -BACK_FILTERSIZE 11 -FLAG_IMAGE \$bpmfit.2.fits
		    awk '{if( \$5 > 1 && \$6 > 1 && \$16 < 1 ){print}}' $name.sub.sex1 > $name.sub.sex
...
#	endif
	...
	set vflag = 13
	if ( 1 ) then
	    set vflag = `awk 'BEGIN{print '$vflag' + 64 + 128 }'`
	endif

	echo "fsimsurfit vflag = $vflag"
...
# BITMASK 1+ + +8+16+  +64+   +2048, include these mask values in the final mask.   
# BADBITS 1+2+4+8+16+32+64+128     , exclude these values from creating a background mesh
	set BITMASK = 25
	if ( $interpolation == 1 ) then
	    set BITMASK = `awk 'BEGIN{print '$BITMASK'+64 }'`
	endif
	if ( 1 ) then
	    set BITMASK = `awk 'BEGIN{print '$BITMASK'+2048 }'`
#        during sky creation above, I want to keep the border in hopes of subtracting it, here when modeling the sky, I dont want to include it.  
	    set BADBITS = `awk 'BEGIN{print '$BADBITS'+2048 }'`
	endif

	$fsiraf/x_fsimsurfit -i $tempdir/$name2:t.txt -f "${name2:t}_mef" -c $scols -r $srows -l $OBJT -h $OBJT -n $niter -g $ngrow -v $vflag  -t 1 -w $wavelet -s $surfit[1] $surfit[2] $surfit[3] -m $minarea -d $dlim -a $maxval -b $minval -x $BS -y $BS -p $fsflat -j $BADBITS -k $BITMASK
	if ( $status != 0 ) then
	    if ( $tryagain != 0 ) then
		echo x_fsimsurfit failed... Trying again from the beginning.
		set REDO = 1
		@ tryagain --
		goto beginning
	    endif
...
# 0 0 0 0 1 0 0 = 4  make weight map and flag map for sextractor
...
# FLAG IMAGE 0 for good, 1 for bad pixels, based on bitmask , help sextractor avoid bad detections.  
	    if ( $status != 0 ) then
