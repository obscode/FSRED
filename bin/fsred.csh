...
set clients = ( vizier.u-strasbg.fr axel.u-strasbg.fr  )
...
#		cp $name.ahead $name:h/$index.ahead
	...
	$scamp @ztmp.cat -c $fsast/scamp.config -MATCH $MATCH -SOLVE_ASTROM $SOLVEAST3 -SOLVE_PHOTOM Y -ASTREF_BAND $AFILT -SN_THRESHOLDS $SNT -ASTREF_WEIGHT $ASW -CROSSID_RADIUS $CID -MOSAIC_TYPE $MOTYPE -STABILITY_TYPE $STABILITY_TYPE -DISTORT_KEYS $DISTORT_KEYS -DISTORT_GROUPS $DISTORT_GROUPS -DISTORT_DEGREES $DISTORT_DEGREES -CHECKPLOT_DEV $checkplot -REF_SERVER $CDSCLIENT -POSITION_MAXERR $POSITION_MAXERR -POSANGLE_MAXERR $POSANGLE_MAXERR -ASTRCLIP_NSIGMA $ASTRCLIP_NSIGMA -PHOTCLIP_NSIGMA $PHOTCLIP_NSIGMA -FLAGS_MASK $FLAGS_MASK -ASTRINSTRU_KEY $ASTRINSTRU_KEY -ASTREF_CATALOG $ASTREF_CATALOG -ASTREFCAT_NAME $ASTREFCAT_NAME -VERBOSE_TYPE FULL >>& fs_scamp.txt &
