...
int main(int argc, char *argv[])
    {
      ...
      int verbose = 0;
      fitsfile *infptr1=NULL, *infptr2=NULL, *outfptr1=NULL, *outfptr2=NULL;  /* pointer to the FITS files */
      char *str=NULL, *list=NULL, *wlist=NULL;
      char line[256], name[256], name2[256], outname[256], outname2[256],suffix[256], *maxvali=""; 
      int status = 0, bitpix, hdunum, nkeys, i, j, _i, _j, ii, jj, _ii, _jj, kk, naxis, size = 0 ,xnum;  /* MUST initialize status */
      long naxes[3], fpixel[3];
      double *pix=NULL, *tpix=NULL, *fit=NULL, tmp, back=0, rand, bsig=0, ave, sigma, density, hisig=3, losig=3, chisq, maxval = 1.E33, minval = -1.E33;
      int *tbpix=NULL;
      /* int *bpix=NULL; */
      double *bpix=NULL;
      int nsmooth, xsmooth = 128, ysmooth = 128, xgrid = 1, ygrid = 1, itype=0, chip;
      int BADBITS = 1;
...
/* ####################################################################### */
	...
	/*  set maxval if supplied  */
	if ( strcmp(maxvali,"BAVE")) {
	  maxval = atof(maxvali);
	} else {
	  fits_read_key(infptr1, TDOUBLE, "BAVE", &back, NULL, &status);
	  fits_read_key(infptr1, TDOUBLE, "BSIG", &bsig, NULL, &status);
	  maxval = back + 3*bsig;
	}
	if ( verbose & 1 ) 
	  printf("line: %3d, %s ,weight = %7.1f, maxval=%f \n",nline,name,gsl_vector_get(weight,nline-1),maxval);
