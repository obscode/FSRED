...
int main(int argc, char *argv[])
{
    ...
    double *pix, *tmp, sum = 0., medval = 0., meanval = 0., minval = 1.E33, maxval = -1.E33;
    ...
    if ( !fits_open_image(&fptr, argv[1], READONLY, &status) )
    {
      ...
      /* tmp = (double *) malloc(naxes[1] * sizeof(double)); /\* memory for 1 column *\/ */
	 ...
	 /* gsl_sort (pix, 1, naxes[0]); */
	 /* /\* medval = (medval*(fpixel[1]-1) + gsl_stats_median_from_sorted_data (pix, 1, naxes[0]))/fpixel[1]; *\/ */
	 /* tmp[fpixel[1]-1] = gsl_stats_median_from_sorted_data (pix, 1, naxes[0]); */
      }
      /* gsl_sort (tmp, 1, naxes[1]); */
      /* medval = gsl_stats_median_from_sorted_data (tmp, 1, naxes[1]); */
      /* free(tmp); */
      free(pix);
